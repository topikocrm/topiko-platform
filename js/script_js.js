...
function copyToClipboardSafe() {
    if (window.currentPreviewData) {
        copyToClipboard(window.currentPreviewData);
    } else {
        window.TopikoUtils.showNotification('❌ No preview data available', 'error');
    }
}

function downloadJSONSafe() {
    if (window.currentPreviewData) {
        downloadJSON(window.currentPreviewData);
    } else {
        window.TopikoUtils.showNotification('❌ No preview data available', 'error');
    }
}

function logToConsoleSafe() {
    if (window.currentPreviewData) {
        logToConsole(window.currentPreviewData);
    } else {
        window.TopikoUtils.showNotification('❌ No preview data available', 'error');
    }
}
...
if (typeof window !== 'undefined') {
    ...
    // Safe API Functions
    window.callTopikoAPISafe = callTopikoAPISafe;
    window.copyToClipboardSafe = copyToClipboardSafe;
    window.downloadJSONSafe = downloadJSONSafe;
    window.logToConsoleSafe = logToConsoleSafe;
