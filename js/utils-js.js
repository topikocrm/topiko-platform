/* ========================================
   TOPIKO UTILITIES - ENHANCED WITH NAVIGATION TRACKING (FIXED VERSION)
   ======================================== */
...
window.TopikoUtils = {
    ...
    // ========================================
    // NAVIGATION TRACKING CORE FUNCTIONS (FIXED)
    // ========================================
    ...
    // NEW: Core navigation tracking function (FIXED VERSION)
    trackPageNavigation: async function(toPage, navigationType = 'forward', pageData = {}) {
        ...
        // Clean and validate pageData to prevent 400 errors
        const cleanPageData = this.sanitizePageData(pageData);
        
        // Prepare navigation data with proper null handling
        const navigationData = {
            user_id: window.topikoApp.currentUserId || null,
            session_id: window.topikoApp.sessionId || 'unknown',
            from_page: fromPage || 'unknown',
            to_page: toPage || 'unknown',
            navigation_type: navigationType || 'forward',
            time_spent_seconds: Math.max(0, timeSpentSeconds), // Ensure positive number
            page_sequence_number: window.topikoApp.pageSequenceNumber || 1,
        ...
        if (window.topikoApp.currentPageInteractions && window.topikoApp.currentPageInteractions.length > 0) {
            const interactionSummary = this.getPageInteractionSummary();
            navigationData.page_data = {
                ...navigationData.page_data,
        ...
        const result = await this.batchTrackNavigationSafe(navigationData);
        
        if (result && result.success) {
            this.addNavigationDebugLog(`📊 Page navigation tracked: ${fromPage} → ${toPage} (${timeSpentSeconds}s)`, 'info');
        } else {
            this.addNavigationDebugLog(`❌ Failed to track navigation: ${result?.error || 'unknown error'}`, 'error');
        ...
        window.topikoApp.pageSequenceNumber = (window.topikoApp.pageSequenceNumber || 0) + 1;
        
        // Add to in-memory history
        if (!window.topikoApp.navigationHistory) {
            window.topikoApp.navigationHistory = [];
        }
        ...
        window.topikoApp.pageViews = (window.topikoApp.pageViews || 0) + 1;
    },
    
    // NEW: Sanitize page data to prevent database errors
    sanitizePageData: function(pageData) {
        ...
        Object.keys(pageData).forEach(key => {
            ...
            if (Array.isArray(value)) {
                ...
                cleaned[key] = value.slice(0, 50).map(item => {
            ...
            if (typeof value === 'object' && value !== null) {
                try {
                    ...
                    if (jsonString.length <= 1000) {
                        ...
                        cleaned[key] = { truncated: true, size: jsonString.length };
                    }
                } catch (error) {
                    cleaned[key] = { error: 'serialization_failed' };
                }
                return;
            }
            
            // Default: convert to string
            cleaned[key] = String(value).substring(0, 100);
        });
        
        return cleaned;
    },
    
    // NEW: Safe UTM data extraction
    getUtmDataSafe: function() {
        try {
            ...
            ['utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_term'].forEach(param => {
                ...
                if (value && typeof value === 'string') {
                    // Sanitize and limit UTM parameter length
                    utmData[param] = value.substring(0, 100).replace(/[<>'"]/g, ''); // Remove potentially dangerous characters
                }
            });
            
            return Object.keys(utmData).length > 0 ? utmData : null;
        } catch (error) {
            this.addNavigationDebugLog(`⚠️ UTM data extraction failed: ${error.message}`, 'warn');
            return null;
        }
    },
    ...
    // NEW: Enhanced batch tracking with better error handling
    batchTrackNavigationSafe: async function(navigationData) {
        if (!window.supabase || !window.supabase.from) {
            this.addNavigationDebugLog('❌ Supabase client not available', 'error');
        ...
        if (this.navigationQueue.length >= batchSize) {
            return await this.flushNavigationQueueSafe();
        }
    ...
    // NEW: Safe queue flushing with detailed error handling
    flushNavigationQueueSafe: async function() {
        ...
        try {
            ...
            const validatedBatch = batch.map(item => this.validateNavigationItem(item)).filter(Boolean);
            
            if (validatedBatch.length === 0) {
                this.addNavigationDebugLog('⚠️ No valid navigation items to flush', 'warn');
                return { success: true };
            }
            ...
            this.addNavigationDebugLog(`✅ Batch tracked ${validatedBatch.length} navigation events`, 'info');
            ...
            console.error('Failed navigation batch:', batch);
            
            // Store locally as fallback (don't re-add to queue to avoid infinite loop)
            batch.forEach(nav => this.storeNavigationLocally(nav));
            return { success: false, error: error.message };
        }
    },
    
    // NEW: Validate individual navigation items
    validateNavigationItem: function(item) {
        ...
        const validated = {
            ...
            from_page: String(item.from_page || 'unknown'),
            to_page: String(item.to_page || 'unknown'),
            navigation_type: String(item.navigation_type || 'forward'),
            time_spent_seconds: Math.max(0, parseInt(item.time_spent_seconds) || 0),
            page_sequence_number: Math.max(1, parseInt(item.page_sequence_number) || 1),
        ...
        if (item.user_agent && typeof item.user_agent === 'string') {
            validated.user_agent = item.user_agent.substring(0, 500);
        }
        
        if (item.referrer_url && typeof item.referrer_url === 'string') {
            validated.referrer_url = item.referrer_url.substring(0, 500);
        }
        
        if (item.utm_data && typeof item.utm_data === 'object') {
            validated.utm_data = item.utm_data;
        }
        
        if (item.page_data && typeof item.page_data === 'object') {
            validated.page_data = this.sanitizePageData(item.page_data);
        }
